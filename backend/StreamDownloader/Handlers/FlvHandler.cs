...
public class FlvHandler : IStreamHandler
{
    ...
    public async Task<byte[]> DownloadSegmentAsync(Segment segment, HttpClient httpClient, long speedLimitKb = 0, CancellationToken cancellationToken = default)
    {
        ...
        var maxBytesPerSecond = speedLimitKb * 1024;
        
        // 如果文件较大（>50MB），使用临时文件避免内存溢出
        if (contentLength.HasValue && contentLength.Value > 50 * 1024 * 1024)
        {
            return await DownloadLargeFileAsync(response, segment, maxBytesPerSecond, cancellationToken);
        }

        // 小文件或未知大小的有限流，使用内存缓冲
        await using var stream = await response.Content.ReadAsStreamAsync(cancellationToken);
        var downloadStream = maxBytesPerSecond > 0 ? new ThrottledStream(stream, maxBytesPerSecond) : stream;
    ...
    /// 大文件流式下载到临时文件，避免内存溢出
    /// </summary>
    private async Task<byte[]> DownloadLargeFileAsync(HttpResponseMessage response, Segment segment, long maxBytesPerSecond, CancellationToken cancellationToken)
    {
        var tempFile = Path.GetTempFileName();
        try
        {
            await using var networkStream = await response.Content.ReadAsStreamAsync(cancellationToken);
            var downloadStream = maxBytesPerSecond > 0 ? new ThrottledStream(networkStream, maxBytesPerSecond) : networkStream;
