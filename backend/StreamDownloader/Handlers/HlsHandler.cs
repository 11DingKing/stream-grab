...
public class HlsHandler : IStreamHandler
{
                ...
                try
                {
                    ...
                    if (newSegments.Count > 0)
                    {
                        ...
                        foreach (var segment in newSegments)
                        {
                            ...
                            try
                            {
                                var data = await DownloadSegmentAsync(segment, httpClient, options.SpeedLimit, cancellationToken);
    ...
    public async Task<byte[]> DownloadSegmentAsync(Segment segment, HttpClient httpClient, long speedLimitKb = 0, CancellationToken cancellationToken = default)
    {
        ...
        if (maxBytesPerSecond > 0)
        {
            ...
            await using var throttledStream = new ThrottledStream(networkStream, maxBytesPerSecond);
            using var memoryStream = new MemoryStream();
            await throttledStream.CopyToAsync(memoryStream, cancellationToken);
